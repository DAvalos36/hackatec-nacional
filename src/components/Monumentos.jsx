...
export default function Monumentos(props) {
	const list = [
		{
			title: "Teatro",
			...
			ciudad: "Recorrer",
			estado: "Chihuahua",
			link: "https://parral3.digserv.com.mx/",
		},
		{
			...
			ciudad: "Recorrer",
			estado: "Chihuahua",
			link: "https://parral2.digserv.com.mx/",
		},
		{
			title: "Catedral",
			texto: "Hidalgo del Parral",
			...
			ciudad: "Recorrer",
			estado: "Chihuahua",
			link: "https://parral1.digserv.com.mx/",
		},
		{
			title: "Museo",
			texto: "Campeche",
		...
		},
		{
			...
			ciudad: "Recorrer",
			estado: "Quintana Roo",
			link: "https://hackatec.digserv.com.mx/",
		},
	];

	const [fil, setFiltrados] = useState(list);

	function filtrarPorEstado(arr, estadoBuscado) {
		if (estadoBuscado === "") return arr;
		return arr.filter((item) => item.estado === estadoBuscado);
	}

	useEffect(() => {
		const filtrados = filtrarPorEstado(list, props.estado);
			...
			{fil.map((item, index) => (
				...
				>
					...
					<CardFooter className="text-small justify-between">
						...
						<a href={item.link} target="_blank" rel="Modelos">
							<Button
								variant="bordered"
								className=""
								startContent={
